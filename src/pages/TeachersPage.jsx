...
const emptyForm = { fio: '', position: 'Преподаватель', academic_degree: '', login: '', password: '' };
const positionsList = ['Ассистент', 'Преподаватель', 'Старший преподаватель', 'Доцент', 'Профессор'];
...
export default function TeachersPage() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (editId) { updateTeacher(editId, form); } else { addTeacher(form); }
    setShowModal(false);
  };
  ...
  return (
    <div className="page">
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={openCreate}><Plus size={18} /> Добавить преподавателя</button>
      </div>
      <div className="filters-bar">
        <div className="search-input">
          <Search size={16} />
          <input placeholder="Поиск по ФИО или должности..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      </div>
      <motion.div className="table-container" layout>
        <table>
          <thead><tr><th>ФИО</th><th>Должность</th><th>Учёная степень</th><th>Логин</th><th></th></tr></thead>
          <tbody>
            <AnimatePresence>
              {filtered.map((t, i) => (
                <motion.tr key={t.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ delay: i * 0.03 }}>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: '4px', justifyContent: 'flex-end' }}>
                      ...
                      <button className="btn-delete" onClick={() => { if (confirm(`Удалить преподавателя ${t.fio}?`)) deleteTeacher(t.id); }}><Trash2 size={16} /></button>
                    </div>
                  </td>
                </motion.tr>
              ))}
        ...
        {filtered.length === 0 && (<div className="empty-state"><GraduationCap size={48} /><p>Преподаватели не найдены</p></div>)}
      </motion.div>
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editId ? 'Редактирование преподавателя' : 'Новый преподаватель'}>
        <form onSubmit={handleSubmit}>
          <div className="form-group"><label>ФИО</label>
            <input required value={form.fio} onChange={(e) => setForm(f => ({ ...f, fio: e.target.value }))} placeholder="Фамилия Имя Отчество" /></div>
          <div className="form-row">
            ...
            <div className="form-group"><label>Учёная степень</label>
              <select value={form.academic_degree} onChange={(e) => setForm(f => ({ ...f, academic_degree: e.target.value }))}>
                <option value="">Нет</option><option>Кандидат наук</option><option>Доктор наук</option>
              </select></div>
          </div>
          {!editId && (<div className="form-row">
            <div className="form-group"><label>Логин</label>
              <input required value={form.login} onChange={(e) => setForm(f => ({ ...f, login: e.target.value }))} placeholder="login" /></div>
            <div className="form-group"><label>Пароль</label>
              <input type="password" value={form.password} onChange={(e) => setForm(f => ({ ...f, password: e.target.value }))} placeholder="По умолчанию: 123456" /></div>
          </div>)}
